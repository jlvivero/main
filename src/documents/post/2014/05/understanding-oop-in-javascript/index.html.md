...
Many developers are familiar with the OOP([Object Oriented Programming](http://en.wikipedia.org/wiki/Object-oriented_programming)) concept in several programming languages. Developers who usually write code in C++, Java, Python, Ruby or other class-based languages may be familiar with *class*, and some of them are surprised by the fact that there's no *class* in JavaScript and we don't use it to achieve OOP.

Yes, there's no *class* in JavaScript. Instead of *class*, we use *prototype* to achieve OOP. That's why JavaScript(or other ECMAScript implementations) is called a [prototype-based](http://en.wikipedia.org/wiki/Prototype-based_programming) language. A *prototype* is shared in several objects, and it means that the objects can do the same actions, like class instances in the class-based languages.
...
Function
--------
Before looking into how *prototype* works, we should look into its *function* in JavaScript. JavaScript is a functional programming language, and *function* does lots of thing in it. Basically, *function* does what it does in other languages, as a procedure or method. But in Javascript, it's not only first-classed and higher-ordered, but also used to achieve OOP. It makes some developers think that *function* is *class* in JavaScript, but it's not true. As I said, there's no *class* in JavaScript. *Function* in JavaScript is a more likely *constructor* than *class*. Let's look in more detail.
...
As you can see in the example above, it does something wrong. In detail, `this` refers to the global object, unless it's strict mode. If it's strict mode, it refers to `undefined`. Then, can't we use `this` other than the object methods? You may have seen a code like below.
...
But when it's used with a `new` operator, a magic happens. When a function is called with a `new` operator, an empty object `{}` becomes the *receiver*, or `this`,  of the function, and the object is returned at the end of the function call without any explicit `return` statement.

This is how *function* in JavaScript works as a *constructor* of objects. For sure, you can add some functions into `this`.
...
As you can see in the example above, we can't easily change how the `describe` method works for each `City` object. It's because the method is not shared through the objects, even though they're from the same *constructor*. It doesn't make sense in OOP. How can we make the object methods shared through the objects?
...
We can get a *prototype* object from an object by using `__proto__` or `Object.getPrototypeOf`. But you should use `Object.getPrototypeOf` because `__proto__` is a bit obsolete.
...
Conclusion
----------
Until now, we've covered how a function works as a *constructor* and a *prototype* is shared through several objects. The fact is quite simple, but it's very important for us to know how they really work internally. This is very basic knowledge and many developers may already know it. But I'll be really happy if this article can be helpful for developers who lost their way in the OOP ocean of JavaScript.
